...
import { Activity, AlertTriangle, Droplets, Heart, TrendingDown, TrendingUp, Users } from 'lucide-react';
    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-[13.5px] text-[#6B7A5A] mt-1">Live hospital operations, donation requests, and resource monitoring.</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <StatCard title="Blood Requests" subtitle="Open today" value="18" suffix="active" tag="4 critical" tagVariant="critical" icon={<Droplets size={18} />} />
        ...
        <StatCard title="On-Call Staff" subtitle="Active shift" value="142" suffix="total" tag="Optimal coverage" tagVariant="ok" icon={<Users size={18} />} trend="+8" trendUp />
      </div>
      ...
      <div className="bg-white rounded-xl border border-[#E8E4D8] p-5">
        ...
        <div className="grid grid-cols-4 gap-5">
          {BLOOD_LEVELS.map(item => <BloodLevelBar key={item.type} item={item} />)}
        </div>
      </div>
          ...
          {ACTIVITY_ITEMS.map((item, index) => (
            <div key={`${item.time}-${item.title}`} className="flex gap-3">
              <div className="flex flex-col items-center flex-shrink-0 w-3">
                ...
                {index < ACTIVITY_ITEMS.length - 1 && <div className="w-px flex-1 bg-[#E8E4D8] my-1" />}
              </div>
              <div className="pb-4">
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <div className="flex justify-between items-center">
        ...
        <span className={cn('text-[11.5px] font-semibold', color.text)}>
