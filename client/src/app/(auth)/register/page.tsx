      ...
      try {
        ...
        const res = await fetch(`${apiUrl}/auth/google`, {
          ...
          body: JSON.stringify({ 
            ...
            role: role // Pass the chosen signup role
          }),
        });
        ...
        // Store credentials in localStorage and cookie
  ...
  useEffect(() => {
    ...
    const fetchInviteDetails = async () => {
      try {
        ...
        const res = await fetch(`${apiUrl}/auth/invite-details?token=${token}`);
        const data = await res.json();
        ...
        setError('Failed to load invitation details. Please contact support.');
      } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isInviteFlow && token) {
        res = await fetch(`${apiUrl}/auth/complete-setup`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  if (inviteLoading) {
    return (
      <div className="flex h-screen w-screen items-center justify-center bg-[#fcfdfa]">
        <div className="flex flex-col items-center gap-3">
          <div className="w-12 h-12 rounded-full border-4 border-emerald-100 border-t-emerald-600 animate-spin" />
          <p className="font-syne font-semibold text-emerald-800 text-sm tracking-wide">Verifying invitation...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="w-full md:w-1/2 lg:w-2/5 flex flex-col h-full overflow-y-auto bg-surface relative">
        ...
        <div className="flex-1 flex flex-col justify-center px-6 md:px-12 py-10 mt-12 md:mt-0 max-w-md mx-auto w-full">
          ...
          <div className="mb-6 text-center md:text-left">
            <h2 className="font-syne font-semibold text-3xl text-on-surface mb-1">
              {isInviteFlow ? 'Complete Your Registration' : 'Create Account'}
            </h2>
            <p className="font-dmsans text-sm text-on-surface-variant">
              {isInviteFlow 
                ? 'Set up a secure password to activate your LifeLink donor account.' 
                : 'Register on the network to access portal tools and diagnostics.'}
            </p>
          </div>
          ...
          <div className="bg-surface-variant rounded-xl p-1 mb-6 flex shadow-[0_4px_15px_rgba(85,107,47,0.05)]">
            {(['Patient', 'Donor', 'Hospital'] as const).map((r) => (
              <button
                ...
                } ${isInviteFlow && role !== r ? 'opacity-50 cursor-not-allowed' : ''}`}
              >
                {r}
              </button>
            ))}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="relative">
                ...
                <input 
                  ...
                  className={`w-full bg-surface-container-lowest border border-outline-variant text-on-surface rounded-lg h-[48px] pl-10 pr-4 focus:ring-1 focus:ring-primary focus:border-primary transition-colors font-dmsans text-base placeholder:text-outline focus:outline-none ${
                    isInviteFlow ? 'bg-neutral-100/80 cursor-not-allowed opacity-75 focus:ring-0 focus:border-outline-variant' : ''
                  }`}
            ...
            >
              {loading ? (isInviteFlow ? 'Activating...' : 'Registering...') : (isInviteFlow ? 'Complete Registration' : 'Register')}
  ...
  return (
    <Suspense fallback={
      <div className="flex h-screen w-screen items-center justify-center bg-[#fcfdfa]">
        <div className="flex flex-col items-center gap-3">
          <div className="w-12 h-12 rounded-full border-4 border-emerald-100 border-t-emerald-600 animate-spin" />
          <p className="font-syne font-semibold text-emerald-800 text-sm tracking-wide">Loading signup page...</p>
        </div>
      </div>
    }>
      <RegisterForm />
